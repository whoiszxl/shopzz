    ...
    <el-card class="add-container">
      <el-form :model="skuForm" :rules="rules" ref="skuRef" label-width="100px" class="skuForm">
        ...
        <el-form-item label="活动名称" prop="name">
          <el-input style="width: 300px" v-model="skuForm.name" placeholder="请输入活动名称"></el-input>
        </el-form-item>

        <el-form-item label="活动描述" prop="descs">
          <el-input style="width: 300px" v-model="skuForm.descs" placeholder="请输入活动描述"></el-input>
        </el-form-item>
        ...
        <el-form-item required label="活动" prop="img">
          <el-upload
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      if (id) {
        axios.get(`/promotion/activity/${id}`).then(res => {
    ...
    const submitAdd = () => {
      skuRef.value.validate((vaild) => {
        if (vaild) {
          ...
          httpOption('/promotion/activity', params).then(() => {
            ElMessage.success(id ? '修改成功' : '添加成功')
            router.push({ path: '/activityList' })
          })
        }
      })
    }
