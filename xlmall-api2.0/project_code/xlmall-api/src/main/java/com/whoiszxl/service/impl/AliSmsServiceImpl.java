...
public class AliSmsServiceImpl implements SmsService {
	...
	public ServerResponse<String> sendVerifyCode(String phoneNumber, int verifyCodeLength) {
		...
		try {
			...
			RedisShardedPoolUtil.setEx(Const.SMS.SMS_ALI_VERIFY_CODE_PREFIX + phoneNumber, verifyCode,
	...
	public ServerResponse<String> sendForgetPwdVerifyCode(String phoneNumber, int verifyCodeLength) {
		...
		try {
			...
			// 发送成功之后需要存储验证码到redis中
			RedisShardedPoolUtil.setEx(Const.SMS.SMS_ALI_FORGET_PWD_PREFIX + phoneNumber, verifyCode,
					Integer.parseInt(PropertiesUtil.getProperty("alisms.verifyCode.expire.time")));
