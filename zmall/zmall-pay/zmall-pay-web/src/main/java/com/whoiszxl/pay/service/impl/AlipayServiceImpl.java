...
public class AlipayServiceImpl implements PayService {
    ...
    private String gateway; //支付宝接口网关地址

    @Value("${alipay.appId}")
    private String appId;//支付宝应用id

    @Value("${alipay.rsaPrivateKey1}")
    private String rsaPrivateKey1;

    @Value("${alipay.rsaPrivateKey2}")
    private String rsaPrivateKey2;

    @Value("${alipay.rsaPrivateKey3}")
    private String rsaPrivateKey3;

    @Value("${alipay.format}")
    private String format;//json格式

    @Value("${alipay.charest}")
    private String charest;//编码
    ...
    private String returnUrl;//支付成功跳转的url

    @Value("${alipay.notifyUrl}")
    private String notifyUrl;//支付结果异步通知的url

    @Override
    public PaymentResponseDTO nativePay(String orderId, String money) {
        //TODO 私钥从yaml读取有问题，无法读取完整，分割读取也不行。
        //String rsaPrivateKey = rsaPrivateKey1 + rsaPrivateKey2 + rsaPrivateKey3;
        ...
        model.setSubject("ZMALL支付");
        //暂用订单ID代替
        model.setBody("为ZMALL订单" + orderId + "支付");
    ...
    public static void main(String[] args) {
        ...
        AlipayTradeWapPayRequest alipayRequest = new AlipayTradeWapPayRequest();
        AlipayTradeWapPayModel model = new AlipayTradeWapPayModel();
        model.setOutTradeNo("100000");//商户的订单，就是闪聚平台的订单
        model.setTotalAmount("1");//订单金额（元）
