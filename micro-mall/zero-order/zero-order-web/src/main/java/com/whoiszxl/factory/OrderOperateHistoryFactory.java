    ...
    public OrderOperateHistory get(Order order, Integer operateType) {
        ...
        switch (operateType) {
            case OrderOperateTypeConstants.CREATE_ORDER:
                operateNote = String.format("完成创建订单，订单编号为：%s", order.getOrderSn());
                break;
            case OrderOperateTypeConstants.CANCEL_ORDER:
                operateNote = String.format("用户取消订单，订单编号为：%s", order.getOrderSn());
                break;
            case OrderOperateTypeConstants.PAY_ORDER:
                operateNote = String.format("用户支付了订单，订单编号为：%s，支付金额为：%s", order.getOrderSn(), order.getPayAmount().toPlainString());
                break;
            case OrderOperateTypeConstants.PRODUCT_DELIVERY:
                operateNote = String.format("订单发货了，订单编号为：%s", order.getOrderSn());
                break;
            case OrderOperateTypeConstants.CONFIRM_RECEIPT:
                operateNote = String.format("用户确认收货了，订单编号为：%s", order.getOrderSn());
                break;
            default:
                break;
        }
